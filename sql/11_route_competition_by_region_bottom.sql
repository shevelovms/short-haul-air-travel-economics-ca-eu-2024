-- Question: What are 10 least competitive US domestic routes in 2024?
...
SELECT
    route_id,
    route_region,
    total_flights_per_route,
    unique_carriers_per_route,
    DENSE_RANK() OVER (PARTITION BY route_region ORDER BY unique_carriers_per_route ASC) AS r
FROM carriers_per_route_domestic
ORDER BY unique_carriers_per_route ASC, total_flights_per_route DESC
LIMIT 10;