-- Purpose: compare carrier-count competition buckets with route-level HHI concentration
-- Intermediate grain: one row per route_id
-- Final output: one row per route_competitive type of 1) number of total routes, 2) average HHI per route, 3) median HHI per route
...
SELECT
    ...
    FLOOR(AVG(route_hhi)) AS avg_route_hhi,
    FLOOR(PERCENTILE_DISC(0.5) WITHIN GROUP (ORDER BY route_hhi)) AS median_route_hhi
FROM route_competition_vs_hhi
GROUP BY route_competition
ORDER BY ARRAY_POSITION(ARRAY['monopoly', 'duopoly', 'competitive'], route_competition);