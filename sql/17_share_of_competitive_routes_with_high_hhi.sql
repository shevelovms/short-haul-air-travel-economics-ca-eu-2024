-- Purpose: define the share of 'competitive' routes with high HHI.
-- Intermediate grain: one row per route_id
-- Final output: one-row percentage of competitive routes with high HHI
...
-- Categorize "competitive" routes by their HHI (high/low to moderate.

competitive_routes AS (
    SELECT
        route_id,
        CASE
            WHEN hhi_ranking = 'high' THEN 'High concentration'
            ELSE 'Low to moderate concentration'
        END AS concentration_group
    FROM route_competition_vs_hhi
    WHERE route_competition = 'competitive'
)

-- Group results by their HHI categories and compute shares.

SELECT
    concentration_group,
    COUNT(*) AS routes,
    ROUND(100.0 * COUNT(*) / SUM(COUNT(*)) OVER (), 1) AS pct_routes
FROM competitive_routes
GROUP BY concentration_group
ORDER BY
    CASE
        WHEN concentration_group = 'High concentration' THEN 1
        ELSE 2
    END;